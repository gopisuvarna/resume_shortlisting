...
export default function JobsPage() {
  ...
  const { jobs, loading, search, jobType, expLevel, setSearch, setJobType, setExpLevel } = useJobsList()
  ...
  return (
    <div className="min-h-screen bg-[var(--surface)]">
      ...
      <div className="relative overflow-hidden"
           style={{ background: 'linear-gradient(150deg,#1a1f6e 0%,#2d3fc8 55%,#3b82f6 100%)' }}>
        {/* Grid overlay */}
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 py-14 sm:py-20 text-center">
          ...
          <h1 className="font-display text-3xl sm:text-5xl font-bold text-white mb-4 text-balance">
            Build your career at<br className="hidden sm:block" />
            <span className="text-blue-200"> {COMPANY}</span>
          </h1>
          <p className="text-blue-200 text-base sm:text-lg mb-8 max-w-xl mx-auto">
            Join a team of passionate engineers, designers, and builders creating the future.
          </p>

          {/* Search bar */}
          <div className="max-w-3xl mx-auto">
            <JobFilters
              search={search} jobType={jobType} expLevel={expLevel}
              onSearch={setSearch} onJobType={setJobType} onExpLevel={setExpLevel}
              heroStyle
            />
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
        ...
        {loading ? (
          ...
          <div className="card p-16 text-center">
            ...
            <p className="font-display font-bold text-slate-700 mb-1">No positions found</p>
