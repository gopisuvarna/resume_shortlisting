...
import { applicationsAPI, extractApiErrorMessage } from "@/lib/api";
...
export default function JobDetailPage() {
  ...
  const handleApply = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!file) {
      setAppError("Please upload a PDF resume.");
      return;
    }
    setAppError("");
    setSubmitting(true);
    try {
      ...
      setAppError(
        extractApiErrorMessage(error, "Failed to submit. Please try again."),
      );
    } finally {
      setSubmitting(false);
    }
            ...
            {[
              ...
              .map((s) => (
                <div key={s.title} className="card p-6 sm:p-8">
                  <h2 className="font-display font-bold text-slate-900 text-lg mb-4">
                    {s.title}
                  </h2>
            ...
            <form onSubmit={handleApply} className="space-y-5">
              ...
              <FormField label="Your Key Skills">
                <SkillInput
                  ...
                  onChange={(s) =>
                    setForm((p) => ({ ...p, skills_mentioned: s }))
                  }
        ...
        {step === "done" && (
          <div className="card p-12 text-center max-w-lg mx-auto fade-in">
            ...
            <p className="text-slate-500 mb-2">
              Your resume has been saved and your application is under review.
            </p>
            <p className="text-slate-400 text-sm mb-8">
              Track your application status on the My Applications page.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link href="/my-applications" className="btn-primary px-6">
                Track My Applications
              </Link>
              <Link href="/jobs" className="btn-secondary px-6">
                Browse More Jobs
              </Link>
            </div>
          </div>
