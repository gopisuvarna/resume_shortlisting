...
export default function MyApplicationsPage() {
  ...
  const handleWithdraw = async (id: number) => {
    ...
    try {
      ...
      alert(extractApiErrorMessage(error, "Could not withdraw.", true));
    } finally {
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm(
        "Permanently delete this application? This removes your data completely.",
      )
    )
      return;
    setDeletingId(id);
    try {
      ...
      alert(extractApiErrorMessage(error, "Could not delete.", true));
    } finally {
      setDeletingId(null);
    }
  };
    ...
    content = (
      <div className="card p-12 text-center">
        ...
        <p className="text-slate-400 text-sm mb-6">
          Apply to open positions and track them here
        </p>
        <Link href="/jobs" className="btn-primary px-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              {apps.length} application{applicationSuffix}
            </p>
          </div>
